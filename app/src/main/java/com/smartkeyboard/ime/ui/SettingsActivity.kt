...
class SettingsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        content.addView(card {
            ...
            addView(switchRow("Auto-koreksi", "Perbaiki typo otomatis (SymSpell + konteks)", prefs.isAutoCorrectEnabled) {
                prefs.isAutoCorrectEnabled = it
            })
            addView(divider())
            addView(switchRow(
                "Auto-kapital",
                "Huruf besar di awal kalimat / setelah . ! ?",
                prefs.isAutoCapitalizeEnabled
            ) {
                prefs.isAutoCapitalizeEnabled = it
            })
            addView(divider())
            addView(switchRow(
                "Auto-spasi tanda baca",
                "Spasi otomatis setelah . , ! ? ; : (undo lewat backspace)",
                prefs.isAutoSpaceAfterPunctEnabled
            ) {
                prefs.isAutoSpaceAfterPunctEnabled = it
            })
            addView(divider())
